...
class TitleScreen extends LitElement {
  ...
  static get properties () {
    return {
      ...
      clientDesc: { type: String },
      crazyText: { type: String },
    }
  }

  constructor () {
    super()
    window.fetch('text.json').then(res => res.json()).then(config => {
      this.discord = config.discord
      this.github = config.github
      this.clientName = config.clientName
      this.clientDesc = config.clientDesc
	  
      var crazyTexts = config.crazyText
	  this.crazyText = crazyTexts[Math.floor(Math.random() * crazyTexts.length)]
	})
  ...
  render () {
    return html`
      <div class="minecraft">
        ...
        <span class="splash">${this.crazyText}</span>
      </div>
      ...
      <div class="menu">
        ...
        <div class="menu-row">
          <pmui-button pmui-width="98px" pmui-label="Discord" @pmui-click=${() => openURL(this.discord)}></pmui-button>
          <pmui-button pmui-width="98px" pmui-label="Github" @pmui-click=${() => openURL(this.github)}></pmui-button>
        </div>
      </div>

      <div class="bottom-info">
        <span>${this.clientName}</span>
        <span>${this.clientDesc}</span>
      </div>
    `
  }
}
